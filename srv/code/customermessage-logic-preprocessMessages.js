 ...
 * @Before(event = { "READ" }, entity = "gnseo_STUDENTH21Srv.CustomerMessage")
...
module.exports = async function(request) {
	...
	try {
		...
		customerMessages = await SELECT.from('gnseo_STUDENTH21.CustomerMessage').forUpdate();
	} catch (error) {
	...
	await Promise.all(customerMessages.map(async customerMessage => {
		...
		if (!titleEnglish || !messageCategory || !messageUrgency || !messageSentiment || !summaryCustomerLanguage || !summaryEnglish || !fullMessageEnglish) {
			...
			try {
				...
				await UPDATE('gnseo_STUDENTH21.CustomerMessage')
